...
playerMovement = playerTransform.InverseTransformDirection(playerMovement);   
    ...
    {        if (playerPosture == PlayerPosture.Stand)  
        ...
        {            animator.SetFloat(postureHash, crouchTreshold, 0.1f, Time.deltaTime);  
            switch (locomotionState)  
            {                case LocomotionState.Idle:  
                    animator.SetFloat(moveSpeedHash, 0, 0.1f, Time.deltaTime);  
                    break;  
                default:  
                    animator.SetFloat(moveSpeedHash, crouchSpeed, 0.1f, Time.deltaTime);  
                    break;  
            }        }        else if (playerPosture == PlayerPosture.Jumping)  
        {            animator.SetFloat(postureHash, midairThreshold, 0.05f, Time.deltaTime);  
            animator.SetFloat(verticalVelHash, VerticalVelocity, 0.05f, Time.deltaTime);  
            animator.SetFloat(feetTweenHash, feetTween);  
        }        else if (playerPosture == PlayerPosture.Landing)  
